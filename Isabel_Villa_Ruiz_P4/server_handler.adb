...
function Unb_Mod_Hash ( Unbounded_In : ASU.Unbounded_String ) return Hash_Range is

    Total: Natural := 0;
    Current_char: Character;
begin

    for Indice in 1..ASU.Length(Unbounded_In) loop
        Current_char := ASU.Element(Unbounded_In, Indice);
        Total := Total + Character'Pos(Current_char);
    end loop;
    --Pruebas
    --ATIO.Put_Line("Hash: " & Hash_Range'Image(Hash_Range'Mod(Total)));
...
begin
    ...
    case Message is
        ...
        when CM.Init =>
            ...
            --ATIO.Put(Boolean'Image(Success));
            ...
            if Success = True then
                ...
                New_Bool := True;
                --NEW IN P4
                -- Comprobacion del cliente nuevo en la lista de inactivos.

                --AÑADIR CLIENTE
                -- ADDED
                ICP.Delete(Inactive_Clients, Nick_Name, Success);

                if Success = True then
                    --El cliente estaba en la lista de inactivos y se reconecta
                    Comment := ASU.To_Unbounded_String(ASU.To_String(Nick_Name) & " rejoins the chat");
                else
                    --El cliente es un nuevo usuario por primera vez y se conecta
                    Comment := ASU.To_Unbounded_String(ASU.To_String(Nick_Name) & " joins the chat");
                end if;
                --
                ...
                exception
                    ...
                    when ACP.Full_Map =>
                        --ATIO.Put_Line("He llegado al full map!");
            ...
            if New_Bool = True then
                --MENSAJE TIPO SERVER PARA COMUNICAR A LOS READERS DEL NUEVO USUARIO.
                --New in p4: depende de si es joins o rejoins.
                Send_To_All_Client(Active_Clients, ASU.To_Unbounded_String("server"),Comment, Nick_Name);
            end if;
